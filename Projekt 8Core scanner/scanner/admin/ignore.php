...
/* Load all counts + current tab records */
$counts = [];
foreach (array_keys($CATEGORIES) as $cat) {
    $counts[$cat] = (int)$pdo->prepare("SELECT COUNT(*) FROM scanner_ignore_list WHERE category=?")->execute([$cat]) ? (int)$pdo->query("SELECT COUNT(*) FROM scanner_ignore_list WHERE category='$cat'")->fetchColumn() : 0;
}
