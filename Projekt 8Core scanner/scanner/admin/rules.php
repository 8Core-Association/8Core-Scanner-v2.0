...
if ($fType !== '' && array_key_exists($fType, $TYPES)) {
    $where[]  = "type = ?";
    $params[] = $fType;
}
if ($fRisk !== '' && in_array($fRisk, $RISKS, true)) {
    $where[]  = "risk = ?";
    $params[] = $fRisk;
}
if ($fActive !== '') {
          ...
          <tr class="<?= $rule['active'] ? '' : 'rule-inactive-row' ?>">
            <td>
              <form method="post" class="inline-form">
                ...
                <button type="submit" class="toggle-btn <?= $rule['active'] ? 'toggle-on' : 'toggle-off' ?>" title="<?= $rule['active'] ? 'Klikni za deaktivaciju' : 'Klikni za aktivaciju' ?>">
