...
.box{background:#1e293b;border:1px solid #334155;border-radius:12px;padding:32px 28px;max-width:440px;text-align:center;}
h2{color:#f87171;margin:0 0 12px;}p{color:#94a3b8;font-size:14px;}</style>
</head><body><div class="box">
<h2>Installer je zaključan</h2>
<p>Instalacija je već provedena. Datoteka <code>install/install.lock</code> postoji.</p>
<p>Za ponovnu instalaciju administratoru treba ručno obrisati <code>install.lock</code>.</p>
<p><a href="../login.php" style="color:#60a5fa;">Idi na prijavu</a></p>
</div></body></html>
    <?php
    exit;
}
...
// Detektira putanju web aplikacije
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $step === 2) {
    // Korak 2: provjera DB i unos putanja
    ...
    if (empty($errors)) {
        // Generiraj config.php
        $configContent = generateConfigPhp($dbHost, $dbName, $dbUser, $dbPass, $dbCharset,
                                           $adminUser, $adminPass,
        ...
        // Generiraj scanner-db.conf.sample sadržaj s popunjenim vrijednostima
        $dbConfContent = generateDbConf($dbHost, $dbName, $dbUser, $dbPass, $dbCharset,
                                        $rootEngPath, $quarPath, $logPath);
        $dbConfPath = __DIR__ . '/generated-scanner-db.conf';
        file_put_contents($dbConfPath, $dbConfContent);
        $success[] = 'Root DB konfiguracija generirana: install/generated-scanner-db.conf';
        $success[] = 'Kopirajte generated-scanner-db.conf u ' . $rootEngPath . '/scanner-db.conf kao root.';
    }
...
function generateConfigPhp($dbHost, $dbName, $dbUser, $dbPass, $dbCharset,
    ...
    $adminPassEsc = addslashes($adminPass);
    return "<?php\n" .
           "/**\n" .
           ...
           "    'default_admin_pass' => '" . $adminPassEsc . "',\n" .
           "\n" .
...
function generateDbConf($dbHost, $dbName, $dbUser, $dbPass, $dbCharset,
                          $rootEngPath, $quarPath, $logPath) {
    return "# 8Core Scanner v2.0 — Root DB konfiguracija\n" .
           "# Generirano installerom: " . date('Y-m-d H:i:s') . "\n" .
           "# Kopirati u: " . $rootEngPath . "/scanner-db.conf\n" .
           "# Postaviti dozvole: chmod 600 scanner-db.conf\n\n" .
           'DB_HOST="' . $dbHost    . '"' . "\n" .
           'DB_NAME="' . $dbName    . '"' . "\n" .
           'DB_USER="' . $dbUser    . '"' . "\n" .
           'DB_PASS="' . $dbPass    . '"' . "\n" .
           'DB_CHARSET="' . $dbCharset . '"' . "\n\n" .
           'ROOT_ENGINE_PATH="' . $rootEngPath . '"' . "\n" .
...
.btn { display:inline-flex; align-items:center; gap:6px; padding:10px 18px; border:none; border-radius:7px; font-family:inherit; font-size:13px; font-weight:600; cursor:pointer; transition:background .12s; }
.btn-primary { background:#2563eb; color:#fff; }
.btn-primary:hover { background:#1d4ed8; }
.btn-ghost { background:transparent; color:#94a3b8; border:1px solid #334155; }
.checks { margin-bottom:0; }
.check-item { display:flex; align-items:center; gap:10px; padding:8px 0; border-bottom:1px solid #1e293b; font-size:13px; }
...
.msgs li { padding:5px 0; font-size:13px; border-bottom:1px solid #1e293b; font-family:'Consolas','Monaco',monospace; }
...
<div class="wrap">
  ...
  <?php if ($step === 3): ?>
    <!-- KORAK 3: GOTOVO -->
    <div class="card">
      <h2>Instalacija uspješno završena</h2>
      ...
      <?php endif; ?>
      <p style="color:#94a3b8;font-size:13px;margin:0 0 8px;">Installer je zaključan (install.lock kreiran).</p>
      <p style="color:#94a3b8;font-size:13px;margin:0 0 16px;">
        Slijedeći korak: Kopirajte <code>install/generated-scanner-db.conf</code> u root engine direktorij
        kao <code>scanner-db.conf</code> i postavite prava <code>chmod 600</code>.
      </p>
      <a href="../login.php" class="btn btn-primary">Idi na prijavu</a>
    </div>

  <?php elseif ($step === 2): ?>
    <!-- KORAK 2: KONFIGURACIJA DB + PUTANJE -->
    <div class="card">
      ...
      <form method="post">
        ...
        <div class="section-sep">Putanje instalacije</div>

        <div class="field">
          <label>Web aplikacija — putanja na serveru</label>
          <input type="text" name="web_app_path" value="<?= htmlspecialchars($_POST['web_app_path'] ?? $detectedWebPath, ENT_QUOTES) ?>">
          <div class="hint">Detektirano automatski. Mijenjajte samo ako nije ispravno.</div>
        </div>
        <div class="field">
        ...
        <div class="field">
          <label>Root engine putanja (ROOT_ENGINE_PATH)</label>
          <input type="text" name="root_eng_path" value="<?= htmlspecialchars($_POST['root_eng_path'] ?? '/root/8core_scanner', ENT_QUOTES) ?>">
          <div class="hint">Direktorij root enginea van web roota. Default: /root/8core_scanner</div>
        </div>
        <div class="field-row">
          <div class="field">
